...
use windows::Win32::Graphics::Direct3D9::{Direct3DCreate9, IDirect3DDevice9, D3DADAPTER_DEFAULT, D3DBACKBUFFER_TYPE_MONO, D3DCREATE_SOFTWARE_VERTEXPROCESSING, D3DDEVTYPE_HAL, D3DDISPLAYMODE, D3DFORMAT, D3DPRESENT_PARAMETERS, D3DSURFACE_DESC, D3DSWAPEFFECT_DISCARD, D3DVIEWPORT9, D3D_SDK_VERSION, D3DRECT};
use windows::Win32::Graphics::Gdi::{ScreenToClient, RGNDATA};
use windows::Win32::System::SystemServices::{D3DCLEAR_TARGET, D3DCOLOR_RGB};
#[cfg(target_arch = "x86")]
...
use windows::Win32::UI::WindowsAndMessaging::{DefWindowProcW, GetCursorPos, GetDesktopWindow, GetForegroundWindow, IsChild, GWLP_WNDPROC, CallWindowProcA, FindWindowExA, GetWindowRect};

use crate::hooks::common::{Fence, imgui_wnd_proc_impl, ImguiWindowsEventHandler};
...
type Dx9ResetFn = unsafe extern "system" fn(this: IDirect3DDevice9, *const D3DPRESENT_PARAMETERS) -> HRESULT;
...
type WndProcType = unsafe extern "system" fn(hwnd: HWND, umsg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT;

////////////////////////////////////////////////////////////////////////////////////////////////////
// Global singletons
////////////////////////////////////////////////////////////////////////////////////////////////////
static mut TRAMPOLINE: OnceCell<(Dx9EndSceneFn, /*Dx9PresentFn,*/ Dx9ResetFn)> = OnceCell::new();
...
// static mut IMGUI_RENDERER_TEST: OnceCell<Mutex<Box<ImguiRenderer>>> = OnceCell::new();

static ENDSCENE_RUNNING: Fence = Fence::new();

unsafe extern "system" fn imgui_dx9_end_scene_impl(this: IDirect3DDevice9) -> HRESULT {
	let _fence = ENDSCENE_RUNNING.lock();
	...
	// Draw square
	let mut rec: D3DRECT = D3DRECT::default();
	rec.x1 = 50; //makes line longer/shorter going lef
	rec.y1 = 50; //base y
	rec.x2 = 200; //makes line longer/shorter going right
	rec.y2 = 200; //makes line one pixel tall
	this.Clear(1, &rec, D3DCLEAR_TARGET as _, 0, 0., 0).expect("Clear error!");

	if let Some(renderer) = IMGUI_RENDERER.get_mut() {
		if let Some(mut renderer) = renderer.try_lock() {
			println!("Inside");
			// renderer.render();
		}
	} else {
		IMGUI_RENDERER.set(Mutex::new(Box::new(ImguiRenderer::new(this.clone()))));
	}

	// let renderer =
	// 	IMGUI_RENDERER.get_or_init(|| Mutex::new(Box::new(ImguiRenderer::new(this.clone()))));
	//
	// renderer.try_lock().expect("Failed to lock renderer").render();
...
unsafe extern "system" fn imgui_wnd_proc(
	...
	match IMGUI_RENDERER.get().map(Mutex::try_lock) {
		...
		Some(None) => {
			...
			println!("IMGUI_RENDERER is currently locked");
...
impl ImguiRenderer {
	unsafe fn new(this: IDirect3DDevice9) -> Self {
		// println!("ImguiRenderer.new");
		let mut ctx = imgui::Context::create();

		let renderer = imgui_dx9::Renderer::new(&mut ctx, this).expect("Failed to create new imgui_dx9 Renderer");
	...
	unsafe fn render(&mut self) {
		...
		if GetWindowRect(self.renderer.get_hwnd(), &mut rect as _).as_bool() {
		...
		IMGUI_RENDER_LOOP.get_mut().expect("Failed to unwrap IMGUI_RENDER_LOOP").render(ui, &self.flags);
		let draw_data = self.ctx.render();

		self.renderer.render(draw_data).expect("Failed to render draw_data!");
	}
...
impl ImguiDx9Hooks {
	...
	{
		println!("ImguiDx9Hooks.new()");
		let (hook_dx9_end_scene_address, /*dx9_present_address,*/ dx9_reset_address, device) =
		...
		Self(MhHooks::new([hook_dx9_end_scene, /*hook_dx9_present,*/ hook_dx9_reset])
...
impl Hooks for ImguiDx9Hooks {
	...
	unsafe fn unhook(&mut self) {
		...
		ENDSCENE_RUNNING.wait();

		if let Some(renderer) = IMGUI_RENDERER.get() {
			renderer.try_lock().expect("Failed to lock renderer").cleanup();
		}
...
unsafe fn get_dx9_present_addr() -> (Dx9EndSceneFn, /*Dx9PresentFn,*/ Dx9ResetFn, IDirect3DDevice9) {
	println!("get_dx9_present_addr()");
